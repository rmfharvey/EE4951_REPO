...
**     Date/Time   : 2015-12-11, 18:22, # CodeGen: 2
**     Abstract    :
...
void main_task(os_task_param_t task_init_data)
{
  ...
  while (1) {
    ...
    /* Write your code here ... */
	  sdspi_status_t result;
	  uint16_t i;
	  uint8_t txArr[512];
	  uint8_t rxArr[512]={0x00};
	  for(i=0; i<512; i++){
		  txArr[i]=i;
	  }

	  while(true)	{
		  result = SDSPI_DRV_WriteBlocks(&memoryCard1_spi_state, &memoryCard1_state, txArr, 10U, 1U);
	 	  result = SDSPI_DRV_ReadBlocks(&memoryCard1_spi_state, &memoryCard1_state, rxArr, 10U, 1U);
	 	  i=0;
	   }
